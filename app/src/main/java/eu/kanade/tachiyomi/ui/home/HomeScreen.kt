	...
	@Composable
	 fun TestContent() {
		...
		NavigationDrawer(
				...
				Column(modifier = Modifier.fillMaxHeight(), verticalArrangement = Arrangement.Center) {
					tabs.forEachIndexed { index, tab ->
						NavigationDrawerItem(
							...
							onClick = { selectedIndex = index }, leadingContent = { tab.options.icon },
						) {
			...
			when(selectedIndex){
					0 -> tabs.get(0).Content()
				1 -> tabs.get(1).Content()
				2 -> tabs.get(2).Content()
				3 -> tabs.get(3).Content()
				else -> Text("what the fuck")
			}
		}
	}
